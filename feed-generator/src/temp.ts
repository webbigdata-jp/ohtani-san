import {
  ...
  async function analyzeText(author: string, text: string): Promise<string | null> {
    const systemPrompt = `You are a helpful assistant that can understand both English and Japanese text. For the given text by ${author}, respond with 'YES' if it contains ANY reference or connection to Shohei Ohtani (大谷翔平), a Japanese baseball player who plays as a pitcher and fielder in the American MLB(Major League Baseball:メジャーリーグ), DODGERS(ドジャーズ). This includes:
